...
class nxcVarnishClearType extends eZWorkflowEventType {
    ...
    public function execute( $process, $event ) {
        ...
        if( count( $nodeIDs ) > 0 ) {
            ...
            foreach( $nodeIDs as $nodeID ) {
                $request = 'ban obj.http.X-eZPublish-NodeID == ' . $nodeID . ' && obj.http.X-eZPublish-InstallationID == ' . $installationID;
                try {
    ...
    public static function getNodeIDs( $object ) {
        ...
        // Append parent nodes (cache manager might skip some)
        $nodeIDs = array_merge( $nodeIDs, $object->parentNodeIDArray() );
